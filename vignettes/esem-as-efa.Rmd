...
## McDonald's Omega
We can calculate McDonald's omega using the fitted model and target rotation matrix. Currently, the use of a target rotation matrix is mandatory.

```{r}
omega_esem(extended_cfa_fit, target_rot)
```
...
Since the function runs lavaan's `summary()` before dumping the results, one can add extra results such as modification indices and r-squared measures with the corresponding modifiers to the
call (the all modifiers are listed in the lavaans `lavaan-class` help).
