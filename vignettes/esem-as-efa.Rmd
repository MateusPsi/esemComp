...
The `esem_efa()` function is actually a wrapper around the `psych` package function `fa()` for exploratory factor analysis. All its controls can be supplied for finer specification of the factor extraction procedure. Just be sure to always input the arguments of the orginal function with `name = value` pairs. Be sure to check `fa()`'s documentation for information on those controls and information about all the fields inside the output object.
